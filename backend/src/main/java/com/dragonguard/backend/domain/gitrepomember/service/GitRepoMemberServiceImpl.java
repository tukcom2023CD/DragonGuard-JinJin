...
public class GitRepoMemberServiceImpl implements GitRepoMemberService {
    ...
    public void updateOrSaveAll(
            final List<GitRepoMemberResponse> gitRepoResponses, final GitRepo gitRepo) {
        final List<GitRepoMember> gitRepoMembers =
                validateAndGetGitRepoMembers(gitRepoResponses, gitRepo);
    ...
    private GitRepoMember findGitRepoMember(final Member member, final GitRepo gitRepo) {
        return gitRepoMemberRepository
                .findByGitRepoAndMember(gitRepo, member)
                .orElseGet(
                        () ->
                                gitRepoMemberRepository.save(
                                        gitRepoMemberMapper.toEntity(member, gitRepo)));
    }

    private void saveOrUpdateGitRepoMembers(final List<GitRepoMember> gitRepoMembers) {
        gitRepoMembers.forEach(
                gitRepoMember ->
                        gitRepoMemberRepository
                                .findByGitRepoAndMember(
                                        gitRepoMember.getGitRepo(), gitRepoMember.getMember())
                                .orElseGet(
                                        () -> {
                                            GitRepoMember savedGitRepoMember =
                                                    gitRepoMemberRepository.save(gitRepoMember);
                                            return gitRepoMemberRepository
                                                    .findById(savedGitRepoMember.getId())
                                                    .orElseThrow(EntityNotFoundException::new);
                                        })
                                .update(gitRepoMember));
    }

    private List<GitRepoMember> validateAndGetGitRepoMembers(
    ...
    private GitRepoMember saveAndGetGitRepoMember(
            final GitRepoMemberResponse response, final GitRepo gitRepo) {
        final String githubId = response.getGithubId();
        final GitRepoMember gitRepoMember =
                gitRepoMemberRepository
                        .findByGitRepoAndMemberGithubId(gitRepo, githubId)
                        .orElseGet(
                                () ->
                                        gitRepoMemberRepository.save(
                                                gitRepoMemberMapper.toEntity(
                                                        response.getMember(), gitRepo)));
    ...
    @Override
    public void saveAllIfNotExists(
            final List<GitRepoMemberResponse> gitRepoResponses, final GitRepo gitRepo) {
        gitRepoMemberRepository.saveAll(findGitRepoMembers(gitRepoResponses, gitRepo));
    }

    private List<GitRepoMember> findGitRepoMembers(
            final List<GitRepoMemberResponse> gitRepoResponses, final GitRepo gitRepo) {
        return gitRepoResponses.stream()
                ...
                .map(
                        gitRepoResponse ->
                                findGitRepoMember(
                                        gitRepoResponse,
                                        findMemberByGitRepoResponse(gitRepoResponse),
                                        gitRepo))
                .collect(Collectors.toList());
    }

    private GitRepoMember findGitRepoMember(
            final GitRepoMemberResponse response, final Member member, final GitRepo gitRepo) {
        final GitRepoMember gitRepoMember =
                gitRepoMemberRepository
                        .findByGitRepoAndMember(gitRepo, member)
    ...
    private Member findMemberByGitRepoResponse(final GitRepoMemberResponse gitRepoMemberResponse) {
        return memberService.saveIfNone(
                gitRepoMemberResponse.getGithubId(),
                AuthStep.NONE,
                gitRepoMemberResponse.getProfileUrl());
    }

    @Override
    public GitRepoMember findByGitRepoAndMemberGithubId(
                                ...
                                gitRepoMemberRepository.save(
                                        gitRepoMemberMapper.toEntity(
                                                memberService.findMemberOrSaveWithRole(
                                                        githubId, Role.ROLE_USER, AuthStep.NONE),
                                                gitRepo)));
    }

    @Override
    public GitRepoMember loadEntity(final Long id) {
        return gitRepoMemberRepository.findById(id).orElseThrow(EntityNotFoundException::new);
    }
