...
public interface BlockchainMapper {
    ...
    @Mapping(target = "createdAt", expression = "java(history.getBaseTime().getCreatedAt().toString())")
    @Mapping(target = "contributeType", expression = "java(history.getBlockchain().getContributeType().toString())")
    ...
    default List<BlockchainResponse> toBlockchainResponseList(final List<Blockchain> blockchains) {
        return toResponseList(blockchains.stream().map(Blockchain::getHistories).flatMap(List::stream).collect(Collectors.toList()));
    }
}
