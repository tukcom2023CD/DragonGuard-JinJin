...
public abstract class ContributionService<T extends Contribution, ID> implements EntityLoader<T, ID> {
    ...
    public void saveContribution(final Member member, final Integer contributionNum, final Integer year, final ContributeType contributeType) {
    ...
    private void updateAndSendTransaction(final Member member, final Integer commitNum, final Integer year, final Blockchain blockchain, final ContributeType contributeType) {
    ...
    private boolean isNotUpdatable(final Integer commitNum, final T contribution, final long newBlockchainAmount) {
    ...
    private T getContribution(final Member member, final Integer year) {
        return contributionRepository.findByMemberAndYear(member, year)
    ...
    private void sendTransaction(final Member member, final Long amount, final Blockchain blockchain, final ContributeType contributeType) {
        if (isInvalidToTransaction(member, amount, blockchain)) {
            return;
        }
        blockchainKafkaProducer.send(new BlockchainKafkaRequest(member.getId(), amount, contributeType));
    }
