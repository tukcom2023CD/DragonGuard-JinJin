...
public class GitRepoClientJobConfig {
    ...
    private final GithubClient<GitRepoBatchRequest, Mono<List<GitRepoMember>>>
            gitRepoMemberBatchClient;
    ...
    public Job clientJob() {
        return jobBuilderFactory.get(CLIENT_JOB).start(step()).preventRestart().build();
    }
    ...
    public CompositeItemProcessor<GitRepo, List<GitRepoMember>> compositeProcessor() {
        CompositeItemProcessor<GitRepo, List<GitRepoMember>> processor =
    ...
    public FunctionItemProcessor<GitRepo, Mono<List<GitRepoMember>>> clientProcessor() {
        return new FunctionItemProcessor<>(
                gitRepo ->
                        gitRepoMemberBatchClient.requestToGithub(
                                new GitRepoBatchRequest(adminApiToken.getApiToken(), gitRepo)));
    }
