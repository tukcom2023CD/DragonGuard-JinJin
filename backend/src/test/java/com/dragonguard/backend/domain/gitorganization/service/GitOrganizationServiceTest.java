...
class GitOrganizationServiceTest extends LoginTest {
    ...
    void saveGitOrganizations() {
        // given
        Set<MemberOrganizationResponse> gitOrganizationNames =
                Set.of(
                        new MemberOrganizationResponse(
                                "tukcom2023CD", "http://githubProfileImage1"),
                        new MemberOrganizationResponse("C-B-U", "http://githubProfileImage2"));

        // when
        gitOrganizationService.findAndSaveGitOrganizations(
                gitOrganizationNames,
                memberRepository.findById(authService.getLoginUserId()).orElseThrow());

        long count =
                gitOrganizationNames.stream()
    ...
    void findGitOrganizationByGithubId() {
        // given
        List<GitOrganization> given =
                List.of(
                        GitOrganization.builder().member(loginUser).name("tukcom2023CD").build(),
        ...
        // when
        List<GitOrganization> gitOrganization =
                gitOrganizationService.findGitOrganizationByMember(
    ...
    void loadEntity() {
        // given
        GitOrganization given =
                jpaGitOrganizationRepository.save(
                        GitOrganization.builder().name("tukcom2023CD").member(loginUser).build());
