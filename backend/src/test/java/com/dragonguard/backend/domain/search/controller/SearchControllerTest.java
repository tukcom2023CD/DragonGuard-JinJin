...
class SearchControllerTest extends RestDocumentTest {
    @MockBean private SearchResultFacade searchService;

    @Test
    @DisplayName("검색 결과 조회")
    void getSearchResult() throws Exception {
        // given
        List<UserResultSearchResponse> expected =
                Arrays.asList(
    ...
    void getSearchResultByFiltering() throws Exception {
        // given
        List<GitRepoResultResponse> expected =
                List.of(
                        new GitRepoResultResponse(
                                1L,
        ...
        perform.andDo(print())
                .andDo(
                        document(
                                "get search result by filtering",
                                getDocumentRequest(),
                                getDocumentResponse()));
    }
}
