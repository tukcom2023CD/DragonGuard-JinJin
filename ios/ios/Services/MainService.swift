        ...
        DispatchQueue.main.async {
            AF.request(url)
                ...
                .responseDecodable(of: MainDecodingModel.self) { response in
                    ...
                    self.result = MainModel(id: data.id, name: data.name ?? "unknown", githubId: data.githubId, commits: data.commits ?? 0, tier: data.tier, authStep: data.authStep, profileImage: data.profileImage ?? "",rank: data.rank)
                }
        }
